...
public class BarajaServicio {
    ...
    public ArrayList<Carta> darCartas(Baraja baraja, int cant){
        ArrayList<Carta> cartas = baraja.getCartas();
        ArrayList<Carta> darCartas = new ArrayList<>();
//        for (int i = 0; i < cant; i++) {
//            darCartas.add(cartas.get(i));
//            cartas.remove(i);
//        }
        for (int i = 0; i < cant; i++) {
            darCartas.add(cartas.get(0));
            cartas.remove(0);
        }
        return darCartas;
    }
    ...
    public void cartasMonton(ArrayList<Carta> monton){
        if(monton.isEmpty()){
            System.out.println("Aun no ha salido ninguna carta de la baraja.");
        }else{
